...
export async function POST(req: NextRequest) {
  ...
  try {
        ...
        for (const item of cartItems) {
          ...
          await tx.stockReservation.create({
            data: {
              ...
              expiresAt: new Date(Date.now() + 10 * 60 * 1000), // 10 minutes reservation
            },
          });

          // Check again to prevent over reservations
    ...
    // 3️⃣ Re-fetch with product details (safe now)
    ...
    const lineItems = [
      ...
      { name: "Tax", quantity: 1, amount: taxValue * 100, currency: "PHP" },
      { name: "Shipping", quantity: 1, amount: shipping, currency: "PHP" },
    ];
